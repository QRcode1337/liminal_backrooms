...
from config import AI_MODELS, SYSTEM_PROMPT_PAIRS
...
class NetworkGraphWidget(QWidget):
        ...
        for i, node_id in enumerate(node_list):
            ...
            for other_id in node_list[i + 1 :]:
                ...
                if distance < min_distance * 2:
                    ...
                    other_vx, other_vy = new_velocities.get(
                        other_id, self.node_velocities.get(other_id, (0, 0))
                    )
                    new_velocities[other_id] = (
                        other_vx - nx * strength,
                        other_vy - ny * strength,
                    )
            ...
            for edge in self.edges:
                ...
                if (source, target) in self.growing_edges and self.growing_edges[
                    (source, target)
                ] < 1.0:
                    continue
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(
            QPen(QColor(COLORS["border"]).darker(150), 0.5, Qt.PenStyle.DotLine)
        )
        grid_size = 40
        ...
        for edge in self.edges:
            ...
            if source in self.node_positions and target in self.node_positions:
                ...
                if growth_progress < 1.0:
                    ...
                    actual_dst_x = (
                        screen_src_x + (screen_dst_x - screen_src_x) * growth_progress
                    )
                    actual_dst_y = (
                        screen_src_y + (screen_dst_y - screen_src_y) * growth_progress
                    )
                else:
                ...
                source_color = QColor(
                    self.node_colors.get(source, self.node_colors_by_type["main"])
                )
                target_color = QColor(
                    self.node_colors.get(target, self.node_colors_by_type["main"])
                )
                ...
                for i in range(num_filaments):
                    ...
                    distance = math.sqrt(
                        (actual_dst_x - screen_src_x) ** 2
                        + (actual_dst_y - screen_src_y) ** 2
                    )
                    ...
                    for j in range(1, num_segments):
                        ...
                        angle = (
                            math.atan2(
                                actual_dst_y - screen_src_y, actual_dst_x - screen_src_x
                            )
                            + math.pi / 2
                        )
                    ...
                    gradient = QLinearGradient(
                        screen_src_x, screen_src_y, actual_dst_x, actual_dst_y
                    )
                ...
                if growth_progress == 1.0:  # Only for fully grown edges
                    ...
                    for j in range(1, num_nodes):
                        ...
                        painter.drawEllipse(
                            QPointF(node_x, node_y), node_size, node_size
                        )
        ...
        for node_id in self.nodes:
            if node_id in self.node_positions:
                ...
                # Get node properties
                node_color = self.node_colors.get(
                    node_id, self.node_colors_by_type["branch"]
                )
                ...
                for i in range(num_points + 1):
                    ...
                    if i == 0:
                        ...
                        control_angle = start_angle + (
                            (i - 0.5) * 2 * math.pi / num_points
                        )
                ...
                for i in range(hyphae_count):
                    ...
                    length = base_length * (
                        1.0 + (random.random() - 0.5) * self.hyphae_variation
                    )
                    ...
                    if random.random() > 0.5:
                        ...
                        painter.drawEllipse(
                            QPointF(end_x, end_y), small_node_size, small_node_size
                        )
    ...
    def get_node_at_position(self, pos):
        ...
        for node_id in self.nodes:
            if node_id in self.node_positions:
                ...
                distance = math.sqrt(
                    (pos.x() - screen_x) ** 2 + (pos.y() - screen_y) ** 2
                )
                if distance <= radius:
                    return node_id
...
class NetworkPane(QWidget):
    ...
    def __init__(self):
        ...
        title.setStyleSheet(
            "color: #D4D4D4; font-size: 14px; font-weight: bold; font-family: 'Orbitron', sans-serif;"
        )
        layout.addWidget(title, alignment=Qt.AlignmentFlag.AlignCenter)

        # Network view - set to expand to fill available space
        self.network_view = NetworkGraphWidget()
        self.network_view.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding
        )
        layout.addWidget(
            self.network_view, 1
        )  # Add stretch factor of 1 to make it expand
...
class ControlPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        """)
        self.auto_image_checkbox.setToolTip(
            "Automatically generate images from AI responses using OpenAI's GPT-image-1 model"
        )
        ...
        """)
        self.view_html_button.clicked.connect(
        ...
        """)
        self.view_full_html_button.setToolTip(
            "View the full conversation in dark mode with greentext styling"
        )
        self.view_full_html_button.clicked.connect(
...
class ConversationPane(QWidget):
    ...
    def setup_ui(self):
        ...
        self.conversation_display.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self.conversation_display.customContextMenuRequested.connect(
        ...
        self.input_field.setPlaceholderText(
            "Seed the conversation or just click propagate..."
        )
        ...
        layout.addWidget(
            self.conversation_display, 1
        )  # Main conversation area gets most space
    ...
    def render_conversation(self):
        ...
        html = "<style>"
        html += "body { font-family: 'Segoe UI', sans-serif; font-size: 10pt; line-height: 1.4; }"
        html += ".message { margin-bottom: 10px; padding: 8px; border-radius: 4px; }"
        ...
        for part in parts:
            if part.startswith("```") and part.endswith("```"):
                ...
                for inline_part in inline_parts:
                    if (
                        inline_part.startswith("`")
                        and inline_part.endswith("`")
                        and len(inline_part) > 2
                    ):
                        # This is inline code
    ...
    def append_text(self, text, format_type="normal"):
        ...
        if format_type in self.text_formats:
            self.conversation_display.setCurrentCharFormat(
                self.text_formats[format_type]
            )
    ...
    def display_conversation(self, conversation, branch_data=None):
        ...
        if is_branch:
            ...
            self.title_label.setText(
                f"{branch_emoji} {branch_type.capitalize()}: {selected_text[:30]}..."
            )
            self.info_label.setText("Branch conversation")
        else:
            ...
            if selected_filter == "Full HTML Document (*.html)":
                ...
                if os.path.exists(full_html_path):
                    ...
                    main_window.statusBar().showMessage(
                        f"Full HTML document exported to {file_name}"
                    )
                    return
                else:
...
class LiminalBackroomsApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.splitter.setChildrenCollapsible(
            False
        )  # Prevent panes from being collapsed
    ...
    def connect_signals(self):
        ...
        # Export button
        self.left_pane.control_panel.export_button.clicked.connect(
    ...
    def on_node_hover(self, node_id):
        ...
        if node_id == "main":
            ...
            self.statusBar().showMessage(
                f"{branch_type.capitalize()}: {selected_text[:50]}..."
            )
    ...
    def on_branch_select(self, branch_id):
        ...
        try:
            ...
            # Update status bar
            self.statusBar().showMessage(
                f"Switched to {branch_data['type']} branch: {branch_id}"
            )
    ...
    def create_branch(
        self, selected_text, branch_type="rabbithole", parent_branch=None
    ):
        ...
        try:
            ...
            parent_id = (
                parent_branch
                if parent_branch
                else (self.active_branch if self.active_branch else "main")
            )
            ...
            if hasattr(self, "process_branch_conversation"):
                ...
                QTimer.singleShot(
                    100, lambda: self.process_branch_conversation(branch_id)
                )
    ...
    def node_clicked(self, node_id):
        ...
        if node_id == "main":
            ...
            visible_conversation = [
                msg for msg in conversation if not msg.get("hidden", False)
            ]
